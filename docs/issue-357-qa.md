...
## Follow-up: restore the headline tracker

The blackout Supply & demand action is removed. Immediately after the countdown, the bar again tracks a compact headline requirement: authored reliability or customer retention when available, otherwise the existing prominent requirement. Detailed thresholds and timing remain in tooltips and All requirements. Reliability uses completed event months and retains the incomplete-history caveat. Cash is concise (for example, Cash: $329M).

Validation passed: 14 mission/evidence browser checks with four intentional viewport skips across desktop, 390 px and 320 px; two final desktop copy checks; 14 focused unit tests; production build; and the full repository check (types, lint, formatting, 122 covered suites / 1,172 tests, and all 18 headless scenario invariants). The removed blackout-link navigation assertions now exercise the retained projected-risk link, including Interties-to-Plants and repeated focus. Final desktop/light and mobile/dark screenshots were reviewed and uploaded to the PR; temporary images were removed.

## Follow-up: target-first status and responsive controls

Headline requirements now place the target first and current evidence in parentheses, including cash, retention, reliability, decision gates and chronic-blackout fallback. Cash uses ≥ $0, preserving the actual failure threshold. An accessible 40 px desktop / 44 px coarse-pointer info button replaces visible All requirements text. Desktop grid and mission status share one 42 px row. The duplicate blackout glyph beside the date is removed; phone cash/date typography and spacing are adjusted, with stacked readouts on the narrowest screens. Facility reorder arrows are hidden on coarse-pointer or mobile-width layouts and remain available on desktop.

Two design subagents reviewed desktop and mobile in two feedback rounds. Desktop reviews covered 1024/1280/1440 px, light/dark, blackout and long authored targets, and accepted both rounds. Mobile reviews covered 320/390 px, 150% text and 768 px coarse-pointer. Round 1 identified an orphaned info-icon row; the coder grouped copy and icon while preserving touch size. Round 2 accepted the phone fix and requested natural tablet wrapping; the coder limited full-width grouping to ≤599 px. Parent verification confirmed the final tablet row returned from 97 px to 53 px, both header and upcoming link aligned, info opened, and reorder arrows were hidden. A separate existing 150% text Facilities header Storage/tab overlap was identified in unchanged pane-header rules and recorded without broadening this change.

Final browser checks passed 14 active cases with four intentional viewport skips, covering responsive layout, info-dialog focus, target copy, hidden mobile/visible desktop reorder controls, shared desktop status row and existing evidence/purchase/navigation flows. Focused tests passed 55 cases initially and 14 helper/header cases after the final fallback-copy completion. The full repository check passed: 122 covered suites / 1,172 tests and all 18 headless scenario invariants. Final type/lint/format checks and production build also passed after the last copy changes. Reviewed final desktop/light and mobile/dark screenshots were uploaded to PR #358, then the uploaded temporary files were removed.

## Follow-up: divider alignment, equal mobile rows, and headline audit

The desktop grid-status section consumes the exact first pane track width, shared from DesktopPanes through a layout-scoped CSS variable derived from its live weights and splitter width. Its boundary follows pointer dragging, keyboard resizing, persisted layouts, resets and window changes between two and three panes. The outer timing, grid and compact mission rows each measure 56 px on 320/390 px mobile layouts; longer mission content remains free to grow.

Six targeted browser checks passed for desktop movement/persistence and exact mobile heights in both themes. The final full mission/evidence browser suite passed 15 active cases with six intentional viewport skips. Parent screenshot verification after dragging found both status boundary and divider at x=548 px. The production build passed. Reviewed resized-desktop/light and equal-height mobile/dark screenshots were uploaded and their temporary files removed.

The parent opened all 11 regular scenarios to verify actual rendered headline selection. Data Center Boom showed Customers ≥ 14,850 (16,501); Deep Freeze, Heatwave + Drought, Sudden Nuclear Shutdown and Wildfire Emergency each showed Demand served ≥ 100% (pending). Carbon Fee, The Shale Boom, Paradise, Rise of Renewables, Hurricane Season and The End of an Era showed Cash ≥ $0 with their respective current cash values. Those six lack an authored numeric reliability/retention objective and intentionally use the primary survival requirement; the bar does not invent bespoke metrics from briefing prose. Headline-selection logic was unchanged by this alignment update.

The final repository check passed: typecheck, lint, formatting, 122 covered Jest suites / 1,172 tests and all 18 headless scenario invariants.
