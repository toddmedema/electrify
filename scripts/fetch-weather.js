...
function offshoreBinaryPath(id) {
  return path.join(OUT_DIR, `${id}.v2.bin`);
}

function hasRequiredWeather(city) {
  const file = city.offshore
    ? offshoreBinaryPath(city.id)
    : binaryPath(city.id);
 ...
 * An offshore city additionally has a .v2.bin; its plain .bin remains a v1 compatibility asset
 * for clients that were open while the newer build was deployed.
 */
function writeIndex(fetched) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        comment:
          "Generated by scripts/fetch-weather.js - every city the game can actually be played in, being exactly the ones with a .bin beside this file. Offshore data uses .v2.bin while .bin remains compatible with old clients. Do not edit by hand.",
 ...
 */
function encodeWeather(rows, includeOffshore = true) {
  const hasOffshore = rows.every(
    (row) => typeof row.windOffshoreKph === "number",
  );
  if (
    !hasOffshore &&
  ...
  const offshore = includeOffshore && hasOffshore;
  ...
  buffer.writeUInt8(offshore ? VERSION : 1, 4);
  ...
  try {
    for (let at = 0; at < wanted.length; at += CITIES_PER_BATCH) {
      ...
      batch.forEach((city) => {
        ...
        // Never change the layout at the legacy URL: an older client may fetch it after this
        // deploy. New clients select the separately named v2 asset through location.offshore.
        fs.writeFileSync(binaryPath(city.id), encodeWeather(cityRows, false));
        if (city.offshore) {
          fs.writeFileSync(
            offshoreBinaryPath(city.id),
            encodeWeather(cityRows),
          );
        }
