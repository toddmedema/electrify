...
power and what it costs. The table below records the original archetype calibration before utility access allocations
were separated from wire size. It describes the relative roles, not current deliverable MW:
current outlooks apply the independent supply allocation and physical line constraint, so
upgrading a wire need not improve its import room. See `docs/intertie-balance.md` for the
scenario portfolio measurements.
